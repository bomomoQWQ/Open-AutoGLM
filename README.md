...
> Open-AutoLLM Agent - 基于视觉语言模型的手机自动化 Agent
...
Open-AutoLLM Agent 是一个基于视觉语言模型的手机端智能助理框架，能够以多模态方式理解手机屏幕内容，并通过自动化操作帮助用户完成任务。
...
如果你使用 Claude Code（或者其他什么东西），配置 [GLM Coding Plan（原项目建议）](https://bigmodel.cn/glm-coding)或者任何支持多模态模型的 Coding Plan 后，输入以下提示词即可快速部署（但是我没试过喵）：
...
详细接口文档和 Python 集成示例见 [SKILL.md](SKILL.md)（我同时把skill打了一个包放在Phone_Agent_2.zip里面了）。
...
| 阿里百炼 | `https://dashscope.aliyuncs.com/compatible-mode/v1` | `qwen-3.5` 等 |
...
Phone Agent 支持 50+ 款主流中文应用：（应该都行🤔）
